...
import { CommonPackUnpackGitRepoOptions, JWKInterface, PrivateState, Tag } from '../types';
...
export async function addActivePubKeyToPrivateState(id: string, currentPrivateStateTxId: string, wallet: JWKInterface, type: string = 'REPO') {
  ...
  const privateStateTxResponse = await signAndSendTx(JSON.stringify(privateState), privateInputTags, wallet)
